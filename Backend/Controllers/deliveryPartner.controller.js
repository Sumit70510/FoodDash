    ...
    try
     {
       ...
       const deliveryPartner = await DeliveryPartner.findOne(query).select("+password");   
       if(!deliveryPartner)
        {
        ...
        const isPasswordMatch = await bcrypt.compare(password, deliveryPartner.password); 
        if(!isPasswordMatch)
         {
           return res.status(400).json({
            message:"Incorrect Credentials",
     ...
     if ((activeSessions.length+1) > sessionLimit&&force){
      ...
      //   await Session.updateMany(
      //    { ownerId: deliveryPartner._id, ownerType: "DeliveryPartner", isActive: true },
      //    { isActive: false } );
      //  }   
         
      let token = jwt.sign({_id : deliveryPartner._id},process.env.SECRET_KEY,{expiresIn : `${process.env.VALID_TILL}d`}); 
